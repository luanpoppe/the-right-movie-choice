...
- Prompt unificado pede mais candidatos que os 0–3 finais e **uma** chamada à tool. `query` em pt-BR (sem colar o ano); `year` é filtro. Resultados na mesma ordem das queries. A tool devolve `details.id` / `details.imdbId` — no JSON do LLM isso vira `tmdbId` / `imdbId` se `found: true`.
...
## Notas
SPA inalterado. Parse inválido no strip do controller vira 500 do Fastify. GET de exemplos de query segue sem catálogo. Não chamar a tool de novo após miss — o prompt manda acertar o título PT na primeira chamada.
