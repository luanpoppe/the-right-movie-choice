...
- Envelope Zod interno: `movies` 0–3 + `response` nonempty, com `tmdbId`/`imdbId` opcionais (vêm de `details.id` / `details.imdbId` da tool). HTTP público expõe esses ids quando presentes — ver [IDs na recommendation](ids-na-recommendation.md).
