...
- Envelope Zod interno: `movies` 0–3 + `response` nonempty, com `tmdbId`/`imdbId` opcionais (vêm de `details.id` / `details.imdbId` da tool, não de um campo `tmdbId` no payload da TMDB). HTTP público stripa esses ids no controller.
