...
- Provider: `AiMovieRecommendationProvider` recebe `AI` injetado. Uma `callStructuredOutput` (`AiModels.PRIMARY`, `threadId: chatId`, turno atual, `MovieRecommendationSchema`, prompt `MovieRecommendationPrompts.unified()`). `safeParse(result.response)`; schema inválido → `WrongMovieSchemaFromLlmException`. Sem `ai.call`.
- Envelope Zod: `movies` 0–3 + `response` nonempty. Pedido impossível → `movies: []` com texto.
